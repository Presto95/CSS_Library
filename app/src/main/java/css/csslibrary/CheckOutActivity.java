...
public class CheckOutActivity extends AppCompatActivity implements AdapterView.OnItemClickListener{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        listView=(ListView)findViewById(R.id.list_view_checkout);
        textView=(TextView)findViewById(R.id.text_checkout);
    ...
    public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
        ...
        builder.setPositiveButton("확인", new DialogInterface.OnClickListener() {
            ...
            public void onClick(DialogInterface dialog, int which) {
                ...
                String id = sp.getString("id", null);
                try {
                    task1.execute(type_checkout, title_checkout, author_checkout,id);
                    //목록 업데이트 코드 들어가야함
                    ...
                    adapter=new Adapter(CheckOutActivity.this,R.layout.list_checkin,items);
                    listView.setAdapter(adapter);
                    Toast.makeText(CheckOutActivity.this,"반납하였습니다.",Toast.LENGTH_SHORT).show();
                } catch (Exception e) {}
            }
    ...
    class CustomTask1 extends AsyncTask<String, Void, String> {
        ...
        protected String doInBackground(String... strings) {
            try {
                ...
                conn.setRequestMethod("POST");//데이터를 POST 방식으로 전송합니다.
                OutputStreamWriter osw = new OutputStreamWriter(conn.getOutputStream());
                sendMsg = "type="+strings[0]+"&title="+strings[1]+"&author="+strings[2]+"&id="+strings[3];//보낼 정보인데요. GET방식으로 작성합니다. ex) "id=rain483&pwd=1234";
                //회원가입처럼 보낼 데이터가 여러 개일 경우 &로 구분하여 작성합니다.
                osw.write(sendMsg);//OutputStreamWriter에 담아 전송합니다.
                osw.flush();
                //jsp와 통신이 정상적으로 되었을 때 할 코드들입니다.
